...
- Prior to TypeScript version 4.2, type alias names [_may_ appear in error messages](https://tinyurl.com/2jjdat8g), sometimes in place of the equivalent anonymous type (which may or may not be desirable). Interfaces will always be named in error messages.
- Type aliases may not participate [in declaration merging, but interfaces can](https://tinyurl.com/2lexbu5y).
- Interfaces may only be used to [declare the shapes of objects, not rename primitives](https://tinyurl.com/2hxq274q).
- Interface names will [_always_ appear in their original form](https://tinyurl.com/2f4x6f9y) in error messages, but _only_ when they are used by name.
