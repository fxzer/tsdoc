...
See [Module: Functions](/declaration-files/templates/module-function) for details of how that works, and the [Modules reference](/declaration-files/templates/module) page.
...
This example is a good case for [using generics](/handbooks/handbook-v1/Generics#generic-types) to provide richer type information:
...
The types which are exported can then be re-used by consumers of the modules using either `import` or `import type` in TypeScript code or <a href="/javascript/JSDoc Reference#import-types">JSDoc imports</a>.
...
To understand how namespaces work in `.d.ts` files read the [`.d.ts`  <a href="/declaration-files/Deep Dive">deep dive</a>].
